...
func init() {
	...
	flag.StringVar(&casket.PidFile, "pidfile", "", "Path to write pid file")
	flag.BoolVar(&casket.Quiet, "quiet", false, "Quiet mode (no initialization output)")
	flag.StringVar(&revoke, "revoke", "", "Hostname for which to revoke the certificate")
	flag.StringVar(&serverType, "type", "http", "Type of server to run")
	flag.BoolVar(&toJSON, "casketfile-to-json", false, "From Casketfile stdin to JSON stdout")
	flag.BoolVar(&version, "version", false, "Show version")
	flag.BoolVar(&validate, "validate", false, "Parse the Casketfile but do not start the server")

	casket.RegisterCasketfileLoader("flag", casket.LoaderFunc(confLoader))
	casket.SetDefaultCasketfileLoader("default", casket.LoaderFunc(defaultLoader))
}

// Run is Casket's main() function.
func Run() {
	flag.Parse()
	...
	if plugins {
		fmt.Println(casket.DescribePlugins())
		os.Exit(0)
	}

	// Check if we just need to do a Casketfile Convert and exit
	checkJSONCasketfile()
	...
	casket.EmitEvent(casket.StartupEvent, nil)

	// Get Casketfile input
	casketfileinput, err := casket.LoadCasketfile(serverType)
	if err != nil {
		mustLogFatalf("%v", err)
	}

	if validate {
		err := casket.ValidateAndExecuteDirectives(casketfileinput, nil, true)
		if err != nil {
			mustLogFatalf("%v", err)
		}
		msg := "Casketfile is valid"
		fmt.Println(msg)
...
// confLoader loads the Casketfile using the -conf flag.
func confLoader(serverType string) (casket.Input, error) {
	if conf == "" {
		return nil, nil
	}

	if conf == "stdin" {
		return casket.CasketfileFromPipe(os.Stdin, serverType)
	}

	var contents []byte
	if strings.Contains(conf, "*") {
		// Let casketfile.doImport logic handle the globbed path
...
// defaultLoader loads the Casketfile from the current working directory.
func defaultLoader(serverType string) (casket.Input, error) {
	contents, err := ioutil.ReadFile(casket.DefaultConfigFile)
	if err != nil {
	...
	return casket.CasketfileInput{
		Contents:       contents,
		Filepath:       casket.DefaultConfigFile,
		ServerTypeName: serverType,
	}, nil
}

// getBuildModule returns the build info of Casket
...
func getBuildModule() *debug.Module {
	...
	if ok {
		// The recommended way to build Casket involves
		// creating a separate main module, which
		// preserves casket a read-only dependency
		// TODO: track related Go issue: https://github.com/golang/go/issues/29228
		for _, mod := range bi.Deps {
			if mod.Path == "github.com/tmpim/casket" {
				return mod
			}
		}
	}
...
func checkJSONCasketfile() {
	if fromJSON {
		...
		casketfileBytes, err := casketfile.FromJSON(jsonBytes)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Converting from JSON failed: %v", err)
			os.Exit(2)
		}
		fmt.Println(string(casketfileBytes))
		os.Exit(0)
	}
	if toJSON {
		casketfileBytes, err := ioutil.ReadAll(os.Stdin)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Read stdin failed: %v", err)
			os.Exit(1)
		}
		jsonBytes, err := casketfile.ToJSON(casketfileBytes)
		if err != nil {
