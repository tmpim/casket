...
var tests = []struct {
	...
	{ // 3
		casketfile: `host1:1234, host2:5678 {
	dir abc {
	}
}`,
...
baz"
	...
	{ // 8
		casketfile: `http://host, https://host {
}`,
...
func TestToJSON(t *testing.T) {
	for i, test := range tests {
		output, err := ToJSON([]byte(test.casketfile))
		if err != nil {
...
func TestFromJSON(t *testing.T) {
	for i, test := range tests {
		...
		if string(output) != test.casketfile {
			t.Errorf("Test %d\nExpected:\n'%s'\nActual:\n'%s'", i, test.casketfile, string(output))
		}
	}
}
