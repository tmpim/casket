...
var (
	...
	// Casket process started this one.
	isUpgrade = os.Getenv("CASKET__UPGRADE") == "1"
...
type Instance struct {
	...
	// casketfileInput is the input configuration text used for this process
	casketfileInput Input
...
// executing the newCasketfile. Upon success, it returns the new
...
func (i *Instance) Restart(newCasketfile Input) (*Instance, error) {
	...
	if newCasketfile == nil {
		newCasketfile = i.casketfileInput
	}
	...
	newInst := &Instance{serverType: newCasketfile.ServerType(), wg: i.wg, Storage: make(map[interface{}]interface{})}

	// attempt to start new instance
	err = startWithListenerFds(newCasketfile, newInst, restartFds)
	if err != nil {
...
// LoadCasketfile loads a Casketfile by calling the plugged in
// Casketfile loader methods. An error is returned if more than
// one loader returns a non-nil Casketfile input. If no loaders
// load a Casketfile, the default loader is used. If no default
// loader is registered or it returns nil, the server type's
// default Casketfile is loaded. If the server type does not
// specify any default Casketfile value, then an empty Casketfile
// is returned. Consequently, this function never returns a nil
// value as long as there are no errors.
func LoadCasketfile(serverType string) (Input, error) {
	// If we are finishing an upgrade, we must obtain the Casketfile
	...
	if IsUpgrade() {
		...
		return loadedGob.Casketfile, nil
	}

	// Ask plugged-in loaders for a Casketfile
	cdyfile, err := loadCasketfileInput(serverType)
	if err != nil {
		return nil, err
	}
	...
	// Still nil? Geez.
	if cdyfile == nil {
		cdyfile = CasketfileInput{ServerTypeName: serverType}
	}
...
// CasketfileFromPipe loads the Casketfile input from f if f is
// not interactive input. f is assumed to be a pipe or stream,
// such as os.Stdin. If f is not a pipe, no error is returned
// but the Input value will be nil. An error is only returned
// if there was an error reading the pipe, even if the length
// of what was read is 0.
func CasketfileFromPipe(f *os.File, serverType string) (Input, error) {
	fi, err := f.Stat()
...
func startWithListenerFds(cdyfile Input, inst *Instance, restartFds map[string]restartTriple) error {
	...
	// the caskettls package, so it can perform cert
...
func executeDirectives(inst *Instance, filename string,
	directives []string, sblocks []casketfile.ServerBlock, justValidate bool) error {
			...
			for j, key := range sb.Keys {
				...
				if tokens, ok := sb.Tokens[dir]; ok {
					controller := &Controller{
						instance:  inst,
						Key:       key,
						Dispenser: casketfile.NewDispenserTokens(filename, tokens),
...
func loadServerBlocks(serverType, filename string, input io.Reader) ([]casketfile.ServerBlock, error) {
	validDirectives := ValidDirectives(serverType)
	serverBlocks, err := casketfile.Parse(filename, input, validDirectives)
	if err != nil {
		return nil, err
	}
	if len(serverBlocks) == 0 && serverTypes[serverType].DefaultInput != nil {
		newInput := serverTypes[serverType].DefaultInput()
		serverBlocks, err = casketfile.Parse(newInput.Path(),
...
func Stop() error {
	...
	for {
		...
		// casketmain/run.go blocks until this server instance has shut down
		inst.wg.Add(1)
		defer inst.wg.Done()
...
// CasketfileInput represents a Casketfile as input
// and is simply a convenient way to implement
// the Input interface.
type CasketfileInput struct {
	Filepath       string
	Contents       []byte
...
func (c CasketfileInput) Body() []byte { return c.Contents }

// Path returns c.Filepath.
func (c CasketfileInput) Path() string { return c.Filepath }

// ServerType returns c.ServerType.
func (c CasketfileInput) ServerType() string { return c.ServerTypeName }

// Input represents a Casketfile; its contents and file path
// (which should include the file name at the end of the path).
// If path does not apply (e.g. piped input) you may use
// any understandable value. The path is mainly used for logging,
// error messages, and debugging.
type Input interface {
	// Gets the Casketfile contents
	Body() []byte
...
// DefaultInput returns the default Casketfile input
