...
func TestServeHTTP(t *testing.T) {
	...
	for i, test := range tests {
		...
		ctx := context.WithValue(request.Context(), casket.CtxKey("original_url"), *request.URL)
		request = request.WithContext(ctx)
		ctx = context.WithValue(request.Context(), casket.CtxKey("path_prefix"), test.stripPathPrefix)
		request = request.WithContext(ctx)
...
const (
	testDirPrefix = "casket_fileserver_test"
