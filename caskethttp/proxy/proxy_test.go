...
func TestReverseProxyMaxConnLimit(t *testing.T) {
	...
	su, err := NewStaticUpstreams(casketfile.NewDispenser("Testfile", strings.NewReader(`
		proxy / `+backend.URL+` {
...
func TestUnixSocketProxy(t *testing.T) {
	...
	dir, err := ioutil.TempDir("", "casket_proxytest")
	if err != nil {
...
func testReverseProxyTransparentHeaders(t *testing.T, remoteAddr, forwardedForHeader string, expected []string) {
	...
	// make proxy
	upstreams, err := NewStaticUpstreams(casketfile.NewDispenser("Testfile", strings.NewReader(config)), "")
	if err != nil {
...
func TestReverseProxyLargeBody(t *testing.T) {
	...
	su, err := NewStaticUpstreams(casketfile.NewDispenser("Testfile", strings.NewReader(`proxy / `+backend.URL)), "")
	if err != nil {
		t.Fatal(err)
	}
