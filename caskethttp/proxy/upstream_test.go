...
func TestParseBlockCACertificates(t *testing.T) {
	...
	for i, test := range tests {
		u := staticUpstream{}
		c := casketfile.NewDispenser("Testfile", strings.NewReader(test.config))
		for c.Next() {
...
func TestStop(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			upstreams, err := NewStaticUpstreams(casketfile.NewDispenser("Testfile", strings.NewReader(fmt.Sprintf(config, backend.URL, test.intervalInMilliseconds))), "")
			if err != nil {
...
func TestParseBlockTransparent(t *testing.T) {
	...
	for i, test := range tests {
		upstreams, err := NewStaticUpstreams(casketfile.NewDispenser("Testfile", strings.NewReader(test.config)), "")
		if err != nil {
...
func TestHealthCheckHost(t *testing.T) {
	...
	for i, test := range tests {
		upstreams, err := NewStaticUpstreams(casketfile.NewDispenser("Testfile", strings.NewReader(test.config)), test.host)
		if err != nil {
			t.Errorf("Expected no error. Got: %s", err.Error())
		}
		for _, upstream := range upstreams {
...
func TestHealthCheckPort(t *testing.T) {
	...
	t.Run("valid_port", func(t *testing.T) {
		...
		for i, test := range tests {
			_, err := NewStaticUpstreams(casketfile.NewDispenser("Testfile", strings.NewReader(test.config)), "")
			if err == nil {
...
func TestHealthCheckContentString(t *testing.T) {
	...
	for i, test := range tests {
		u, err := NewStaticUpstreams(casketfile.NewDispenser("Testfile", strings.NewReader(test.config)), "")
		if err != nil {
