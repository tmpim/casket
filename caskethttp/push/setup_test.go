...
func TestPushAvailable(t *testing.T) {
	err := setup(casket.NewTestController("http", "push /index.html /available.css"))
...
func TestConfigParse(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t2 *testing.T) {
			actual, err := parsePushRules(casket.NewTestController("http", test.input))
...
func TestSetupInstalledMiddleware(t *testing.T) {
	...
	// given
	c := casket.NewTestController("http", `push /index.html /test.js`)
...
func TestSetupWithError(t *testing.T) {
	// given
	c := casket.NewTestController("http", "push {\nmethod\n}")
