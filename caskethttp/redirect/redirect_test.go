...
func TestRedirect(t *testing.T) {
	for i, test := range []struct {
		...
		// help with redirects because of Casket's design: a redirect middleware
		...
		re := Redirect{
			...
			Rules: []Rule{
				...
				// redirect rule with Casket because http and https schemes have different listeners,
				// so they don't share a redirect rule. So although these tests prove something
				// impossible with Casket, it's extra bulletproofing at very little cost.
