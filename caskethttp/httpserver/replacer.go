...
func (r *replacer) getSubstitution(key string) string {
	...
	case "{mitm}":
		if val, ok := r.request.Context().Value(casket.CtxKey("mitm")).(bool); ok {
			if val {
				return "likely"
			}
			return "unlikely"
	...
	case "{tls_protocol}":
		if r.request.TLS != nil {
			if name, err := caskettls.GetSupportedProtocolName(r.request.TLS.Version); err == nil {
				return name
			} else {
	...
	case "{tls_cipher}":
		if r.request.TLS != nil {
			if name, err := caskettls.GetSupportedCipherName(r.request.TLS.CipherSuite); err == nil {
				return name
			} else {
