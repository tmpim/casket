...
func TestInspectServerBlocksWithCustomDefaultPort(t *testing.T) {
	Port = "9999"
	filename := "Testfile"
	ctx := newContext(&casket.Instance{Storage: make(map[interface{}]interface{})}).(*httpContext)
	input := strings.NewReader(`localhost`)
	sblocks, err := casketfile.Parse(filename, input, nil)
	if err != nil {
...
func TestGetConfig(t *testing.T) {
	...
	con := casket.NewTestController("http", "")
	con.Key = "foo"
	cfg := GetConfig(con)
	con.Key = "FOO"
	cfg2 := GetConfig(con)
...
// Test to make sure we are correctly hiding the Casketfile
func TestHideCasketfile(t *testing.T) {
	ctx := newContext(&casket.Instance{Storage: make(map[interface{}]interface{})}).(*httpContext)
	ctx.saveConfig("test", &SiteConfig{
		Root:             Root,
		originCasketfile: "Testfile",
	})
	err := hideCasketfile(ctx)
	if err != nil {
		t.Fatalf("Failed to hide Casketfile, got: %v", err)
		return
	}
	if len(ctx.siteConfigs[0].HiddenFiles) == 0 {
		t.Fatal("Failed to add Casketfile to HiddenFiles.")
		return
	}
	...
	t.Fatal("Casketfile missing from HiddenFiles")
}
