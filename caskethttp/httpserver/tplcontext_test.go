...
func TestInclude(t *testing.T) {
	...
	tests := []struct {
		...
		{
			fileContent:          `hello {{ casket }}`,
			expectedContent:      "hello casket",
			shouldErr:            false,
			expectedErrorContent: "",
		},
	}

	TemplateFuncs["casket"] = func() string { return "casket" }
...
func TestFiles(t *testing.T) {
	...
	for i, test := range tests {
		...
		if test.fileNames != nil {
			dirPath, err = ioutil.TempDir(fmt.Sprintf("%s", context.Root), "casket_ctxtest")
			if err != nil {
