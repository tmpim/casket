...
func makeTLSConfig(group []*SiteConfig) (*tls.Config, error) {
	...
	return caskettls.MakeTLSConfig(tlsConfigs)
}
...
func (s *Server) Listen() (net.Listener, error) {
	...
	// Very important to return a concrete casket.Listener
	// implementation for graceful restarts.
	return cln.(casket.Listener), nil
}
...
func (s *Server) Serve(ln net.Listener) error {
	if s.Server.TLSConfig != nil {
		...
		s.tlsGovChan = caskettls.RotateSessionTicketKeys(s.Server.TLSConfig)
	}
...
func (s *Server) serveHTTP(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	c := context.WithValue(r.Context(), casket.CtxKey("path_prefix"), pathPrefix)
	r = r.WithContext(c)
...
// and any relevant information, assuming casket.Quiet == false.
...
func (s *Server) outputSiteInfo(isProcessLog bool) {
	for _, site := range s.sites {
		...
		if casket.IsLoopback(s.Address()) && !casket.IsLoopback(site.Addr.Host) {
...
func (ln tcpKeepAliveListener) Accept() (net.Conn, error) {
	...
	// https://github.com/tmpim/casket/pull/2787
...
// File implements casket.Listener; it returns the underlying file of the listener.
...
// requested host is not served by Casket on a given port.
...
const OriginalURLCtxKey = casket.CtxKey("original_url")
