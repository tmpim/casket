...
-----END CERTIFICATE-----`)
	...
	protocol, _ := caskettls.GetSupportedProtocolName(request.TLS.Version)
	cipher, _ := caskettls.GetSupportedCipherName(request.TLS.CipherSuite)
...
func TestSet(t *testing.T) {
	...
	repl.Set("host", "getcasket.com")
	repl.Set("method", "GET")
	repl.Set("status", "201")
	repl.Set("variable", "value")

	if repl.Replace("This host is {host}") != "This host is getcasket.com" {
...
func TestPathRewrite(t *testing.T) {
	...
	request, err := http.NewRequest("POST", "http://getcasket.com/index.php?key=value", reader)
	if err != nil {
		t.Fatalf("Request Formation Failed: %s\n", err.Error())
	}
	urlCopy := *request.URL
