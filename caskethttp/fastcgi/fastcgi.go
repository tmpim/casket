...
func (h Handler) buildEnv(r *http.Request, rule Rule, fpath string) (map[string]string, error) {
	...
	pathPrefix, _ := r.Context().Value(casket.CtxKey("path_prefix")).(string)
	...
	if r.TLS != nil {
		...
		for k, v := range caskettls.SupportedCiphersMap {
...
// It is parsed from the fastcgi directive in the Casketfile, see setup.go.
type Rule struct {
...
// Note that these are slightly different from SupportedProtocols in caskettls/config.go's
