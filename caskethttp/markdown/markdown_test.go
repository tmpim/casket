...
func TestTemplateReload(t *testing.T) {
	...
	c := casket.NewTestController("http", `markdown {
	...
	err = ioutil.WriteFile(targetFile, []byte("casket"), 0644)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	if expect, got := "hello <p>casket</p>\n", get(); expect != got {
		t.Fatalf("Expected body:\n%q\nbut got:\n%q", expect, got)
	}
