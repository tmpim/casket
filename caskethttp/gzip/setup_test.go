...
func TestSetup(t *testing.T) {
	c := casket.NewTestController("http", `gzip`)
	err := setup(c)
	if err != nil {
	...
	for i, test := range tests {
		_, err := gzipParse(casket.NewTestController("http", test.input))
...
func TestShouldAddResponseFilters(t *testing.T) {
	configs, err := gzipParse(casket.NewTestController("http", `gzip { min_length 654 }`))
