...
func init() {
	casket.RegisterPlugin("websocket", casket.Plugin{
		ServerType: "http",
		Action:     setup,
	})
}
...
func setup(c *casket.Controller) error {
	...
	GatewayInterface = casket.AppName + "-CGI/1.1"
	ServerSoftware = casket.AppName + "/" + casket.AppVersion
...
func webSocketParse(c *casket.Controller) ([]Config, error) {
	var websocks []Config
	...
	for c.Next() {
		...
		cmd, args, err := casket.SplitCommandAndArgs(command)
		if err != nil {
			return nil, err
		}
