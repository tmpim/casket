...
/*
// TODO
func TestCasketStartStop(t *testing.T) {
	casketfile := "localhost:1984"

	for i := 0; i < 2; i++ {
		_, err := Start(CasketfileInput{Contents: []byte(casketfile)})
		if err != nil {
...
func (h *CallbackTestContext) InspectServerBlocks(name string, sblock []casketfile.ServerBlock) ([]casketfile.ServerBlock, error) {
	return sblock, nil
}
...
func TestCasketRestartCallbacks(t *testing.T) {
	for i, test := range []struct {
		...
		_, err := c.instance.Restart(CasketfileInput{Contents: []byte(""), ServerTypeName: serverName})
		if err != nil {
