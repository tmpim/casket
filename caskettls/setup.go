...
func init() {
	...
	casket.RegisterPlugin("tls", casket.Plugin{Action: setupTLS})
...
func setupTLS(c *casket.Controller) error {
	...
	if !ok {
		return fmt.Errorf("no caskettls.ConfigGetter for %s server type; must call RegisterConfigGetter", c.ServerType())
	}
	config := configGetter(c)
	if config == nil {
		return fmt.Errorf("no caskettls.Config to set up for %s", c.Key)
	}
...
func loadCertsInDir(cfg *Config, c *casket.Controller, dir string) error {
...
func makeClusteringPlugin() error {
	...
	if atomic.CompareAndSwapInt32(&clusterPluginSetup, 0, 1) {
		clusterPluginName := os.Getenv("CASKET_CLUSTERING")
		...
		if ok {
			...
			return fmt.Errorf("unrecognized cluster plugin (was it included in the Casket build?): %s", clusterPluginName)
		}
	}
	return nil
}
...
func constructDefaultClusterPlugin() (certmagic.Storage, error) {
	return &certmagic.FileStorage{Path: casket.AssetsPath()}, nil
}
